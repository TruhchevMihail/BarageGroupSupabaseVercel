...
ASSET_CSV_TEMPLATE_HEADERS = ['№', 'name', 'brand', 'model', 'category', 'asset_type', 'serial_number', 'current_location']
...
def build_asset_csv_template():
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(ASSET_CSV_TEMPLATE_HEADERS)
    writer.writerow(['INV-001', 'Къртач', 'Bosch', 'GSH 11', 'Къртач', 'Машина', 'SN123', 'Централен склад'])
    return output.getvalue()
...
def _validate_row(row_number, row, seen_inventory, locations_by_ref):
    ...
    if not inventory_number:
        ...
        errors.append('Дублиран инвентарен № в CSV файла.')
    else:
...
def parse_asset_csv_upload(uploaded_file):
    ...
    if not uploaded_file or not getattr(uploaded_file, 'filename', ''):
        preview.errors.append('Изберете CSV файл.')
        return preview
    ...
    try:
        ...
        preview.errors.append('CSV файлът трябва да бъде UTF-8.')
        return preview

    reader = csv.DictReader(io.StringIO(text))
    if not reader.fieldnames:
        preview.errors.append('CSV файлът няма заглавен ред.')
        return preview

    reader.fieldnames = [_normalize_header(header) for header in reader.fieldnames]
    if 'inventory_number' not in reader.fieldnames:
        preview.errors.append('CSV файлът трябва да съдържа колона № или inventory_number.')
        return preview
    ...
    for rows_seen, row in enumerate(reader, start=1):
        if rows_seen > ASSET_CSV_MAX_ROWS:
            preview.errors.append(f'CSV файлът съдържа повече от {ASSET_CSV_MAX_ROWS} реда.')
            break
    ...
    if rows_seen == 0:
        preview.errors.append('CSV файлът няма редове за импорт.')
    return preview
...
def apply_asset_csv_import(preview, actor):
    ...
    for row in preview.valid_rows:
        ...
        details = f'CSV импорт от {actor.full_name if actor else "администратор"}.'
