...
def normalize_database_url(raw_url):
    ...
    Supabase dashboard examples often start with postgres://. SQLAlchemy prefers
    postgresql://, and Supabase connections should use SSL in production.
    """
...
def load_service_invoice_map():
    if not os.path.exists(SERVICE_INVOICE_MAP):
        return {}
    try:
        with open(SERVICE_INVOICE_MAP, 'r', encoding='utf-8') as handle:
...
def save_service_invoice_map(data):
    with open(SERVICE_INVOICE_MAP, 'w', encoding='utf-8') as handle:
...
location_technicians = db.Table('location_technicians',
...
class Location(db.Model):
    ...
    technical_lead_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)
...
def resolve_upload_fs_path(file_reference):
    ...
    uploads_root = os.path.normpath(os.path.abspath(UPLOAD_FOLDER))
...
def assets():
    ...
    if category:
        query = query.filter(Asset.name.ilike(f'%{category}%'))
    if asset_type:
        query = query.filter(Asset.name.ilike(f'%{asset_type}%'))
    if condition:
    ...
    categories = [
        ...
        .filter(Asset.name.isnot(None), Asset.name != '')
        .distinct()
        .order_by(Asset.name)
        .all()
    ]
...
@app.route('/assets/<int:asset_id>/service', methods=['POST'])
@login_required
...
@app.route('/transfer/<int:asset_id>', methods=['POST'])
@login_required
...
@app.route('/locations/<int:location_id>/unarchive', methods=['POST'])
@login_required
@roles_required(ROLE_SUPERUSER)
@sensitive_rate_limited
...
@app.route('/uploads/asset-image', methods=['POST'])
@login_required
@sensitive_rate_limited
...
@app.route('/assets/<int:asset_id>/move', methods=['GET'])
@login_required
...
@app.route('/assets/<int:asset_id>/service/new', methods=['GET', 'POST'])
@login_required
...
@app.route('/assets/<int:asset_id>/service/<int:record_id>/edit', methods=['GET', 'POST'])
@login_required
...
@app.route('/assets/<int:asset_id>/service/<int:record_id>', methods=['GET'])
@login_required
...
@app.route('/assets/<int:asset_id>/service/<int:record_id>/delete', methods=['POST'])
@login_required
@sensitive_rate_limited
